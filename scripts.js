...
function createElements(data) 
{
   ...
   async function populateSearch(query) {
      ...
      const url = 'https://api.themoviedb.org/3/search/movie?api_key=cd2088a09a0263d493c9587c229ec168&query=' + query.replaceAll(/\s/g, '+');
   ...
   /*
      ...
      2. The second parameter of the .addEventListener() method accepts a function. 
         In this case, () => { ... }is defining an anonymous arrow function (ie doesn't have a name.).
   ...
   */
   ...
   window.addEventListener('DOMContentLoaded', () => {
      ...
      if (query) {
         ...
         const url = `?query=${query.replaceAll(/\s/g, '+')}`;//replaces all whitespace character with "+"
...
searchBarForm.addEventListener("submit", (eventObject) => 
{
   ...
   {
      populateSearch(query);
      const url = `?query=${query.replaceAll(/\s/g, '+')}`;//replaces all whitespace characters with "+"
      history.pushState({ page: 'search', query: query }, null, url);
      searchBar.value = "";
